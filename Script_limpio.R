...
#### Fig tree
  ...
  tree_down <- genesDEdown100 %>%  
    hchart(type = "treemap", hcaes(x = Symbol, value = abs(logFC), color = adj.P.Val)) |>
    hc_title(text ="Genes sub expresados organizados de acuerdo a su LogFC") |>
  ...
  tree_up <- genesDEup100 %>%  
    hchart(type = "treemap", hcaes(x = Symbol, value = logFC, color = adj.P.Val)) |>
    hc_title(text ="Genes sobre expresados organizados de acuerdo a su LogFC") |>
    hc_colorAxis(stops = color_stops(colors = viridis::inferno(10)))
